...
namespace NinjaBay.Domain.Entities
{
    public class Product
    {
        ...
        public ICollection<ProductOrder> Orders { get; } = new List<ProductOrder>();

        public bool IsAvailabe(Product product)
        {
            return product.Quantity > 0 && product.IsActive;
        }
        ...
        public static Product New(string name, string description, decimal? price = null, int? quantity = null)
        {
            return new Product
            {
                Id = Guid.NewGuid(),
        ...
        {
            foreach (var keyWordId in keyWordIds) KeyWords.Add(ProductKeyWord.New(Id, keyWordId));
        }
